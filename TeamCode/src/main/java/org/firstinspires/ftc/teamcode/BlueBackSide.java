...
@Config
@Autonomous(name = "Blue Back Side", group = "Autonomous")
public class BlueBackSide extends LinearOpMode {
    ...
    public void fullRotation(float numOfRotations, float magazinePower, boolean assignNewColor) {
        ...
        while (opModeIsActive() && Math.abs((int)(floatTargetPosition) - magazine.getCurrentPosition()) > tolerance) {
            ...
            double pid = controller.calculate((int)(floatTargetPosition), currentPos);
            /// NEGATIVE FOR FIXING
            // Clamp power
            power = Math.max(-1.0, Math.min(1.0, pid));

            telemetry.addData("Current Position", magazine.getCurrentPosition());
            telemetry.addData("Target Position", (int)(floatTargetPosition));
